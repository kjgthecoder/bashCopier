...
class Root:
    ...
    def scrapeDaNumbas(self):
        ...
        for i in range(len(lines)):
            ...
            # Also accounts for space chars preceding the line number if actually copying from 
            # vim editor.  
            num_regex = re.match(r"^\s*(\d+\s*)?", lines[i])
            if (num_regex is not None): 
                num_str = num_regex.group()
                # Splcing the string based on the length of the regex match 
                lines[i] = lines[i][len(num_str):]
            else: 
